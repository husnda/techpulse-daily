...
def format_item_summary_telegram(summary_text):
    if not summary_text:
        return ""
...
def send_feishu_card(webhook_url, digest_data):
    ...
    elements.append({
        ...
        "text": {
            "tag": "lark_md",
            "content": f"📅 **日期**：{date_str}  ·  ⚡ **今日Star**：⭐ {total_stars}  ·  💬 **HN讨论**：{stats.get('total_hn_comments', 0):,} 条"
        }
    })
    ...
    # GitHub section
    gh_lines = ["**🚀 GitHub 开源热点精选 (基于真实 README 提炼)：**"]
    for i, it in enumerate(gh_items, 1):
        stars = f"+{it.get('stars_today', 0):,}"
        lang = it.get('language', 'Dev')
        topic = it.get('topic', '开源')
        ai_txt = format_item_summary_feishu(it.get('ai_summary') or it.get('description', ''))
        gh_lines.append(f"{i}. [{it['full_name']}]({it['url']}) `[{topic} · {lang}]` ⭐ **{stars}**\n{ai_txt}")
    elements.append({
        "tag": "div",
        "text": {
            "tag": "lark_md",
    ...
    # HN section
    hn_lines = ["**🔥 Hacker News 深度热议 (基于 原文与热评 提炼)：**"]
    for i, it in enumerate(hn_items, 1):
        domain = it.get('domain', 'news')
        topic = it.get('topic', '热点')
        ai_txt = format_item_summary_feishu(it.get('ai_summary') or '')
        hn_lines.append(f"{i}. [{it['title']}]({it['url']}) `[{topic} · {domain}]` (🔥 **{it['points']}** / 💬 [{it['comments_count']}评]({it['hn_url']}))\n{ai_txt}")
    elements.append({
        "tag": "div",
        "text": {
            "tag": "lark_md",
...
def send_telegram(bot_token, chat_id, digest_data):
    ...
    lines.append("🚀 <b>GitHub 开源热点 (基于 README 解读)</b>\n")
    for i, it in enumerate(gh_items, 1):
        stars = f"+{it.get('stars_today', 0):,}"
        safe_name = html.escape(it.get('full_name', ''))
        safe_lang = html.escape(it.get('language', 'Code'))
        safe_topic = html.escape(it.get('topic', '开源'))
        
        lines.append(f"<b>{i}. <a href=\"{it['url']}\">{safe_name}</a></b> <code>{safe_topic} · {safe_lang}</code>")
        lines.append(f"   ⭐ 今日 <b>{stars}</b> ｜ 总星 {it.get('stars_total', 0):,}")
        
        ai_txt = format_item_summary_telegram(it.get('ai_summary'))
        if ai_txt:
            lines.append(ai_txt)
        elif it.get('description'):
            lines.append(f"   <i>{html.escape(it['description'][:70])}</i>")
        lines.append("")
        
    lines.append("───────────────────────────────")
    lines.append("🔥 <b>Hacker News 深度热议 (基于 原文与热评)</b>\n")
    for i, it in enumerate(hn_items, 1):
        safe_title = html.escape(it.get('title', ''))
        safe_domain = html.escape(it.get('domain', 'news'))
        safe_topic = html.escape(it.get('topic', '热点'))
        points = it.get('points', 0)
        comments = it.get('comments_count', 0)
        
        lines.append(f"<b>{i}. <a href=\"{it['url']}\">{safe_title}</a></b> <code>{safe_topic} · {safe_domain}</code>")
        lines.append(f"   🔥 <b>{points}</b> pts ｜ 💬 <a href=\"{it['hn_url']}\"><b>{comments}</b> 条讨论 →</a>")
    ...
    lines.append("───────────────────────────────")
    lines.append('🌐 <a href="https://husnda.github.io/techpulse-daily/">查看完整早报 (共 24 条)</a> ｜ 📡 <a href="https://husnda.github.io/techpulse-daily/feed.xml">RSS 订阅</a>')
    ...
    try:
        ...
        for i, it in enumerate(gh_items, 1):
            stars = f"+{it.get('stars_today', 0):,}"
            plain_lines.append(f"{i}. {it['full_name']} [{it.get('topic')} · {it.get('language')}] ⭐{stars}\n   {it.get('ai_summary', '')}\n   {it['url']}")
        plain_lines.append("\n🔥 Hacker News 深度讨论精选：")
        for i, it in enumerate(hn_items, 1):
            plain_lines.append(f"{i}. {it['title']} [{it.get('topic')} · {it.get('domain')}] (🔥{it['points']} / 💬{it['comments_count']}评)\n   {it.get('ai_summary', '')}\n   {it['url']}")
        plain_lines.append("\n----------------------------------")
...
def send_wecom(webhook_url, digest_data):
    ...
    for i, it in enumerate(gh_items, 1):
        stars = f"+{it.get('stars_today', 0):,}"
        ai_txt = format_item_summary_feishu(it.get('ai_summary') or it.get('description', ''))
        lines.append(f"{i}. [{it['full_name']}]({it['url']}) `[{it.get('topic')} · {it.get('language')}]` ⭐ **{stars}**\n{ai_txt}")
    lines.append("\n**🔥 Hacker News 深度讨论：**")
    for i, it in enumerate(hn_items, 1):
        ai_txt = format_item_summary_feishu(it.get('ai_summary') or '')
        lines.append(f"{i}. [{it['title']}]({it['url']}) `[{it.get('topic')} · {it.get('domain')}]` (🔥**{it['points']}** / 💬[{it['comments_count']}评]({it['hn_url']}))\n{ai_txt}")
    lines.append("\n---")
...
def send_dingtalk(webhook_url, secret, digest_data):
    ...
    for i, it in enumerate(gh_items, 1):
        stars = f"+{it.get('stars_today', 0):,}"
        ai_txt = (it.get('ai_summary') or it.get('description', '')).replace('\n', ' ')
        lines.append(f"- [{it['full_name']}]({it['url']}) ⭐**{stars}** `[{it.get('topic')} · {it.get('language')}]`\n  💡 {ai_txt}")
    lines.append("\n**🔥 Hacker News 讨论：**")
    for i, it in enumerate(hn_items, 1):
        ai_txt = (it.get('ai_summary') or '').replace('\n', ' ')
        lines.append(f"- [{it['title']}]({it['url']}) (🔥**{it['points']}** / 💬[{it['comments_count']}评]({it['hn_url']})) `[{it.get('topic')}]`\n  💡 {ai_txt}")
    lines.append("\n---")
    lines.append("[🌐 查看在线完整早报](https://husnda.github.io/techpulse-daily/)  |  [📡 RSS 订阅源](https://husnda.github.io/techpulse-daily/feed.xml)")
